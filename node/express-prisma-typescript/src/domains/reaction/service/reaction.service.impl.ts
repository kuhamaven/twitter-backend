...
export class ReactionServiceImpl implements ReactionService {
  ...
  async deleteReactToPost (reaction: ReactionDTO): Promise<ExtendedReactionDTO | null> {
    const result = await this.repository.deleteReactToPost(reaction)
    if (result == null) throw new ConflictException('reaction did not exist')
    return result
  }
